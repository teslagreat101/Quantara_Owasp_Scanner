...
def scan_file(
    ...
    # ══════════════════════════════════════════════════════════════════
    # ═══  Layer 6: OWASP VULNERABILITY DETECTION ENGINE (v4.0)  ══════
    # ══════════════════════════════════════════════════════════════════
    #
    # This layer runs 100+ vulnerability patterns covering OWASP Top
    # 10:2025, injection, access control, auth, integrity, cloud/IaC,
    # frontend/JS, API security, supply chain, and recon.
    #
    _OWASP_MODES = (
        ScanMode.FULL, ScanMode.OWASP, ScanMode.INJECTION,
        ScanMode.FRONTEND, ScanMode.CLOUD, ScanMode.API,
        ScanMode.RECON, ScanMode.COMPLIANCE,
    )
    if scan_mode in _OWASP_MODES:
        try:
            from quantum_protocol.analyzers.owasp_scanner import scan_owasp
            owasp_findings = scan_owasp(
                content=content,
            ...
            for of in owasp_findings:
                dedup_key = f"{of.file}:{of.line_number}:{of.family.value}"
                if dedup_key not in seen:
                    seen.add(dedup_key)
                    findings.append(of)
        except ImportError:
            logger.warning("OWASP scanner not available — skipping Layer 6")
        except Exception as exc:
            logger.error("OWASP scan error: %s", exc)

    # ══════════════════════════════════════════════════════════════════
    # ═══  Layer 7: DEDICATED DEEP ANALYZERS (v4.0 Enterprise)  ═══════
    # ══════════════════════════════════════════════════════════════════
    #
    # Each dedicated analyzer provides AST-based taint analysis,
    # framework-specific detection, entropy analysis, and advanced
    # heuristics beyond regex pattern matching.
    #
    _ANALYZER_MAP = {
        ScanMode.FULL: ["injection", "frontend", "cloud", "supply_chain", "recon", "sensitive_data",
                        "broken_access", "misconfig", "auth", "design", "integrity", "logging", "exception", "api"],
        ScanMode.OWASP: ["injection", "frontend", "cloud", "sensitive_data",
                         "broken_access", "misconfig", "auth", "design", "integrity", "logging", "exception", "api"],
        ScanMode.INJECTION: ["injection"],
        ScanMode.FRONTEND: ["frontend", "recon"],
        ScanMode.CLOUD: ["cloud", "misconfig"],
        ScanMode.API: ["api", "auth", "design"],
        ScanMode.RECON: ["recon", "frontend", "sensitive_data"],
        ScanMode.COMPLIANCE: ["injection", "cloud", "sensitive_data", "auth", "logging", "misconfig", "integrity"],
    }
    active_analyzers = _ANALYZER_MAP.get(scan_mode, [])

    def _run_analyzer(name, func, *args):
        try:
            results = func(*args)
            for f in results:
                dk = f"{f.file}:{f.line_number}:{f.family.value}"
                if dk not in seen:
                    seen.add(dk)
                    findings.append(f)
        except Exception as exc:
            logger.error("Analyzer %s error: %s", name, exc)

    if "injection" in active_analyzers:
        from quantum_protocol.analyzers.injection_scanner import scan_injections
        _run_analyzer("injection", scan_injections, content, relative, language, scan_mode, context_window)

    if "frontend" in active_analyzers:
        from quantum_protocol.analyzers.frontend_js_analyzer import scan_frontend
        _run_analyzer("frontend", scan_frontend, content, relative, language, scan_mode, context_window)

    if "cloud" in active_analyzers:
        from quantum_protocol.analyzers.cloud_misconfig import scan_cloud
        _run_analyzer("cloud", scan_cloud, content, relative, language, scan_mode, context_window)

    if "supply_chain" in active_analyzers:
        from quantum_protocol.analyzers.supply_chain import scan_supply_chain
        _run_analyzer("supply_chain", scan_supply_chain, content, relative, language, scan_mode, context_window)

    if "recon" in active_analyzers:
        from quantum_protocol.analyzers.endpoint_extractor import scan_recon
        _run_analyzer("recon", scan_recon, content, relative, language, scan_mode, context_window)

    if "sensitive_data" in active_analyzers:
        from quantum_protocol.analyzers.sensitive_data_exposure import scan_sensitive_data
        _run_analyzer("sensitive_data", scan_sensitive_data, content, relative, language, scan_mode, context_window)

    if "broken_access" in active_analyzers:
        from quantum_protocol.analyzers.broken_access import scan_broken_access
        _run_analyzer("broken_access", scan_broken_access, content, relative, language, scan_mode, context_window)

    if "misconfig" in active_analyzers:
        from quantum_protocol.analyzers.misconfig_engine import scan_misconfig_engine
        _run_analyzer("misconfig", scan_misconfig_engine, content, relative, language, scan_mode, context_window)

    if "auth" in active_analyzers:
        from quantum_protocol.analyzers.auth_scanner import scan_auth_scanner
        _run_analyzer("auth", scan_auth_scanner, content, relative, language, scan_mode, context_window)

    if "design" in active_analyzers:
        from quantum_protocol.analyzers.insecure_design import scan_insecure_design
        _run_analyzer("design", scan_insecure_design, content, relative, language, scan_mode, context_window)

    if "integrity" in active_analyzers:
        from quantum_protocol.analyzers.integrity_scanner import scan_integrity_scanner
        _run_analyzer("integrity", scan_integrity_scanner, content, relative, language, scan_mode, context_window)

    if "logging" in active_analyzers:
        from quantum_protocol.analyzers.logging_scanner import scan_logging_scanner
        _run_analyzer("logging", scan_logging_scanner, content, relative, language, scan_mode, context_window)

    if "exception" in active_analyzers:
        from quantum_protocol.analyzers.exception_scanner import scan_exception_scanner
        _run_analyzer("exception", scan_exception_scanner, content, relative, language, scan_mode, context_window)

    if "api" in active_analyzers:
        from quantum_protocol.analyzers.api_security import scan_api_security
        _run_analyzer("api", scan_api_security, content, relative, language, scan_mode, context_window)
...
def compute_overall_security_score(
    ...
    v4.0: Now includes OWASP vulnerability risk.
    """
    crypto_health = max(0, 100 - quantum_risk)
    secrets_health = max(0, 100 - secrets_exposure)
    vuln_health = max(0, 100 - vuln_risk)
    # Weighted: crypto 25%, secrets 25%, vulns 30%, agility 20%
    score = crypto_health * 0.25 + secrets_health * 0.25 + vuln_health * 0.30 + agility * 0.20
    return max(0.0, min(100.0, round(score, 1)))


def compute_vuln_risk_score(findings: list[CryptoFinding]) -> float:
    """Compute OWASP vulnerability risk score (0–100). Higher = worse."""
    vuln_findings = [f for f in findings if f.family.is_vuln]
    if not vuln_findings:
        return 0.0
    weights = {RiskLevel.CRITICAL: 12.0, RiskLevel.HIGH: 7.0,
               RiskLevel.MEDIUM: 3.0, RiskLevel.LOW: 1.0, RiskLevel.INFO: 0.0}
    total = sum(weights.get(f.risk, 0) * f.confidence for f in vuln_findings)
    return min(100.0, round(total, 1))
...
def build_owasp_coverage(findings: list[CryptoFinding]) -> dict[str, int]:
    ...
    coverage: dict[str, int] = {cat.value: 0 for cat in OwaspCategory}
    for f in findings:
        if f.family.is_vuln:
            cat = f.family.vuln_category
            # Map VulnCategory to OwaspCategory
            _MAP = {
                "Access Control": OwaspCategory.A01_BROKEN_ACCESS,
                "Security Misconfiguration": OwaspCategory.A02_SECURITY_MISCONFIG,
                "Cloud/Infrastructure": OwaspCategory.A02_SECURITY_MISCONFIG,
                "Supply Chain": OwaspCategory.A03_SUPPLY_CHAIN,
                "Injection": OwaspCategory.A05_INJECTION,
                "Insecure Design": OwaspCategory.A06_INSECURE_DESIGN,
                "Authentication": OwaspCategory.A07_AUTH_FAILURES,
                "Data Integrity": OwaspCategory.A08_INTEGRITY_FAILURES,
                "Logging/Monitoring": OwaspCategory.A09_LOGGING_FAILURES,
                "Exception Handling": OwaspCategory.A10_EXCEPTION_HANDLING,
            }
            owasp = _MAP.get(cat.value)
            if owasp:
                coverage[owasp.value] += 1
        elif f.family.is_secret:
            coverage[OwaspCategory.A04_CRYPTO_FAILURES.value] = coverage.get(
                OwaspCategory.A04_CRYPTO_FAILURES.value, 0) + 1
        elif f.family.is_quantum_broken or f.family.is_classically_broken:
            coverage[OwaspCategory.A04_CRYPTO_FAILURES.value] = coverage.get(
                OwaspCategory.A04_CRYPTO_FAILURES.value, 0) + 1
    return {k: v for k, v in coverage.items() if v > 0}


def build_vuln_by_category(findings: list[CryptoFinding]) -> dict[str, int]:
    """Count OWASP vuln findings by VulnCategory."""
    counts: dict[str, int] = {}
    for f in findings:
        if f.family.is_vuln:
            cat = f.family.vuln_category.value
...
def build_file_reports(findings: list[CryptoFinding]) -> list[FileReport]:
    ...
    for filepath, ffindings in sorted(by_file.items()):
        ...
        vulns = sum(1 for f in ffindings if f.family.is_vuln)
